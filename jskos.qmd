...
A **registry** is an [item] and a [dataset] with the following optional fields (in addition to
...
serves        [set] of [items]  Specific versions of items served via this API (OPTIONAL)
...
```{.json #lst-service lst-cap="A service"}
{{< include examples/example.service.json >}}
```

Services roughly cover the [class DataService](https://www.w3.org/TR/vocab-dcat-3/#Class:Data_Service) from [Data Catalog Vocabulary](https://www.w3.org/TR/vocab-dcat-3/).

::: {.callout-note}
Services to access [datasets] ([concept schemes], [concordances] and [registries]) can be specified with either field `services` or via a [distribution].

<!--{mermaid}
classDiagram
  direction LR
  Service --> Dataset : serves
  Dataset --> Service : services
  Distribution --> Service : services
  Dataset --> Distribution : distributions
  note for Dataset "ConceptScheme, Concordance, Registry"-->
:::
...
A **concordance** is an [item] and a [dataset] with the following fields (in addition to the
optional fields `@context`, `address`, `altLabel`, `annotations`, `changeNote`,
...
A dataset is collection of [items]. In JSKOS this object type is used to group fields of object types [concept scheme], [registry], and [concordance]. A **dataset** can have the following optional fields:
