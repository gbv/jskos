...
tool          [set] of [item]           Tool used to create or manage the item
...
`source`, `startDate`, `startPlace`, `subjectOf`, `subject`, `tool`, `type`,
`uri`, `url`, `version`, `versionOf`, `memberSet`, `memberList`,
...
`subjectOf`, `subject`, `tool`, `type`, `uri`, `url`, `version`, and
`versionOf`):
...
`publisher`, `rank`, `scopeNote`, `source`, `startDate`, `startPlace`,
`subjectOf`, `subject`, `tool`, `type`, `uri`, `url`, `version`, and
`versionOf`). All fields except `fromScheme` and `toScheme` are optional.
...
`publisher`, `rank`, `scopeNote`, `source`, `startDate`, `startPlace`,
`subjectOf`, `subject`, `tool`, `type`, `uri`, `url`, `version`, and
`versionOf`). All fields except `from` and `to` are optional.
...
- Add item fields `tool` and `version`
