...
these technologies but it does not require to be experienced with more than
...
- [registries], [distributions], and [services] to package and provide data
...
See @fig-types for the class hierarchy of JSKOS data elements and @fig-links for connections between these main [object types].
...
JSKOS has been designed to be easily convertible to and from RDF, but
[mapping from JSKOS data to RDF](#mapping-to-rdf) is no normative part of the specification.
...
[link templates]: #link-template

A **link template** is a Level 2 URI Template as defined in [RFC 6570]. This includes expressions such as `` {var} `` (simple string expansion), `` {+var} `` (reserved expansion), and `` {#var} `` (fragment expansion) to be replaced with the expanded value of a variable. See [expansion of link templates](#expansion-of-link-templates) for examples and how to derive these variables from an [occurrence] objects.
...
* are not [same resources] (have distinct values in field `uri` for members with this field),
* and have at most one member with field [`rank`](#rank) being the string `preferred`.

Applications MAY allow arrays with [same] members by merging or removing elements.

Member objects SHOULD have a field `uri`. Applications MAY restrict sets to
require the field `uri` for all non-null members.

Applications SHOULD NOT change the order of elements in a set.

Applications MAY ignore or disallow the value `null` in sets. If `null` is allowed, sets MUST be interpreted as following to support [closed world statements]:
...
:::{#lst-ex-sets lst-cap="JSKOS sets"}
The following JSON arrays are JSKOS sets:
...
The following JSON arrays are not valid JSKOS sets:
...
[item type]: #resource-and-property-types
[Item types]: #resource-and-property-types

A **resource type** is a [concept] with MANDATORY field `uri`. It is used to
distinguish different kinds of concepts or other [resources]. Concept types are
referred to by their [URI] in array field `type` of a [resource].

A **property type** is a [concept] with MANDATORY field `uri`. It is used to
specify the type of a [qualified statement]. Property types are referenced by
their [URI] in the keys of a [qualified map].
...
An **object type URI** is an item type URI, or one of the following:

kind of data        object type URI
------------------- -----------------------------------------------------
[occurrence]        *not defined yet*
[location]          <http://www.opengis.net/ont/geosparql#Geometry>
[media]             <http://iiif.io/api/presentation/2#Manifest> 
[annotation]        <http://www.w3.org/ns/oa#Annotation>
[address]           <https://schema.org/PostalAddress>
SKOS collections    <http://schema.vocnet.org/NonIndexingConcept>
Resource types      <http://www.w3.org/2002/07/owl#Class>
Properties          <http://www.w3.org/2000/01/rdf-schema#Property>

The latter three are used for subtypes of [concept]. See also [registry] fields
`properties` (used in [qualified statements]) and `types`, and the list of
object type URIs in [dataset] field `objectTypes`.
...
- [resources] for all kinds of entities
    - [items] for named entities      
        ...
        - [dataset] for collections if items
        ...
        - [distributions] for available forms to access items
        - [services] for APIs to access items
    - [occurrences] for uses of concepts
- [concept bundle] for groups of concepts

Object types [concept], [mapping], and [occurrence] are also subtypes of [concept bundle] (see @fig-types).

![hierarchy of main JSKOS object types](types.svg){#fig-types}

![connections between main JSKOS object types](links.svg){#fig-links}

In addition to these main object types, JSKOS defines the following types based on other specifications. These object types are listed as [data types](#sec-data-types) in the previous section:

- [location] for geospatial data (based on GeoJSON)
- [media] for media files (based on IIIF)
- [annotation] to review and comment on resources (based on Web Annotation Data Model)
- [address] for postal addresses (based on vCard and schema.org)

Each object type can identified with an [object type URI](#resource-and-property-types).

::: {.callout-note}
See appendix [object types classification](#object-type-classification) for an encoding of main object types as concept scheme.
:::
...
annotations         [set] of [annotations]                   annotations of the resource or related resources
...
For use cases of field `partOf` see also [Concept Scheme].
...
the optional fields `@context`, `annotations`, `contributor`, `created`,
`creator`, `identifier`, `issued`, `modified`, `partOf`, `publisher`, `rank`,
`source`, `type`, and `uri`):
...
tool          [set] of [item]           Tools used to create or manage the item
issue         [set] of [item]           Individual issues or discussion threads about this item
issueTracker  [set] of [item]           Issue trackers, forums or similar to discuss this item
guidelines    [set] of [item]           Standards or guidelines followed in creation of this item
version       string                    Version number
versionOf     [set] of [item]           Items which this item is a version or edition of
...
Fields `created`, `issued`, `modified`, `creator`, `contributor` and `source` of an item do not refer to the entity referenced by the item but to the item record. For instance an item about the city of Rome might have a recent date `created` while the founding date such as `-0753` would be stated in field `startDate`.
...
`annotations`, `changeNote`, `contributor`, `created`, `creator`, `definition`,
`depiction`, `editorialNote`, `endDate`, `endPlace`, `example`, `guidelines`,
`hiddenLabel`, `historyNote`, `identifier`, `issue`, `issueTracker`, `issued`,
`location`, `modified`, `notation`, `note`, `partOf`, `place`, `prefLabel`,
`publisher`, `rank`, `scopeNote`, `source`, `startDate`, `startPlace`,
`subjectOf`, `subject`, `tool`, `type`, `uri`, `url`, `version`, `versionOf`,
...
[concept schemes]: #concept-scheme
[scheme]: #concept-schemes
[schemes]: #concept-schemes

A **concept scheme** is a [dataset] with the following optional fields (in
addition to the optional fields `@context`, `address`, `altLabel`, `annotations`,
`changeNote`, `contributor`, `created`, `creator`, `definition`, `depiction`, `distributions`,
`editorialNote`, `endDate`, `endPlace`, `example`, `guidelines`, `hiddenLabel`,
`historyNote`, `identifier`, `issue`, `issueTracker`, `issued`, `location`,
`modified`, `notation`, `note`, `partOf`, `place`, `prefLabel`, `publisher`,
`rank`, `objectTypes`, `services`, `scopeNote`, `source`, `startDate`, `startPlace`, `subjectOf`,
`subject`, `tool`, `type`, `uri`, `url`, `version`, and `versionOf`):
...
Field `objectTypes`, if given, SHOULD include the URI <http://www.w3.org/2004/02/skos/core#Concept>.
...
optional fields (in addition to the optional fields `@context`, `annotations`,
`contributor`, `created`, `creator`, `identifier`, `issued`, `modified`,
`partOf`, `publisher`, `source`, `type`, `uri`, `memberSet`, `memberList`,
`memberChoice`, and `memberRoles`):

field        type                   definition
------------ ---------------------- ----------------------------------------------
database     [dataset]              database in which the concepts are used
...
schemes      [set] of [schemes]     Concept Schemes the concepts come from
url          [URL]                  URL of a page with information about the occurrence
template     [link template]        Link template to generate an URL from concepts
separator    string                 Separator to combine multiple concepts in the link template

An occurrence gives the number of a times a concept ("occurrence"), a
combination of concepts ("co-occurrence"), or a set of concepts is used in some
context. The context is typically a database ([dataset]) with entities having some relation
to the concepts -- for instance an occurrence could give the number of documents indexed
with some term in a catalog. 

The field `url`
typically includes a deep link into the database. Applications SHOULD replace the value of field `url` by [link expansion](#expansion-of-link-templates) if both field `template` and one of the [concept bundle] fields (`memberSet`, `memberList`, `memberChoice`, or `memberRoles`) are given.
...
<!-- TODO: specify explicit inference rules for implicitly given concepts etc. -->

:::{#lst-ex2}
The occurrence of DDC classes in K10plus catalogue:

```{.json}
{{< include examples/k10plus.occurrence.json >}}
```
...
:::{#lst-ex-wikidata}
The Wikidata [concept of an individual human](http://www.wikidata.org/entity/Q5) was linked to 206 Wikimedia sites (mostly Wikipedia language editions) and more than 3.7 million people (instances of <http://www.wikidata.org/entity/P31>) at November 15th, 2017.
...
A **registry** is a [dataset] with the following optional fields (in addition to
the optional fields `@context`, `address`, `altLabel`, `annotations`,
`changeNote`, `contributor`, `created`, `creator`, `definition`, `depiction`,
`distributions`,
`editorialNote`, `endDate`, `endPlace`, `example`, `guidelines`, `hiddenLabel`,
`historyNote`, `identifier`, `issue`, `issueTracker`, `issued`, `location`,
`modified`, `notation`, `note`, `partOf`, `place`, `prefLabel`, `publisher`,
`rank`, `services`, `scopeNote`, `source`, `startDate`, `startPlace`, `subjectOf`,
`subject`, `tool`, `type`, `uri`, `url`, `version`, and `versionOf`):
...
properties    [set] of [concepts]        [property types] of [qualified statements] in this registry
annotations   [set] of [annotations]     [annotations] in this registry
languages     [list]                     Supported languages

The first element of array field `type`, if given, MUST be the [item type] URI
<http://www.w3.org/ns/dcat#Catalog> . Applications SHOULD also accept
alternative URI <http://purl.org/cld/cdtype/CatalogueOrIndex> for backwards
compatibility.

Registries are collection of [concepts], [concept schemes], [mappings],
[concordances], [occurrences], and/or other registries with information about
[resource types] and [property types].

A registry limited to only one of the fields `concepts`, `schemes`, `mappings`,
`concordances`, `occurrences`, `registries`, and `annotations` is also called
a **collection** of the corresponding [item type]. Collections MAY contain 
additional resource types (`types`) and property types (`properties`).

```{.json #lst-concept1 lst-cap="A registry with concepts and its subtypes"}
{{< include examples/concept.registry.json >}}
```
...
optional fields `@context`, `address`, `altLabel`, `annotations`, `changeNote`,
...
Distributions roughly cover the class [Distribution](https://www.w3.org/TR/vocab-dcat-3/#Class:Distribution) from [Data Catalog Vocabulary](https://www.w3.org/TR/vocab-dcat-3/).
...
services       [set] of [service] Services to retrieve the distribution (e.g. via an API)
...
A distribution via physical delivery:

```{.json}
{{< include examples/physical.distribution.json >}}
```
:::
...
A **service** is an [item] with the following fields (in addition to the
optional fields `@context`, `address`, `altLabel`, `annotations`, `changeNote`,
`contributor`, `created`, `creator`, `definition`, `depiction`,
`editorialNote`, `endDate`, `endPlace`, `example`, `guidelines`, `hiddenLabel`,
`historyNote`, `identifier`, `issue`, `issueTracker`,  `issued`, `location`,
...
field         type               definition
------------- ------------------ --------------------------------------------------------------------------------
api           [URI]              API type, for instance from BARTOC vocabulary API types vocabulary (RECOMMENDED)
endpoint      [URI]              API endpoint (RECOMMENDED, SHOULD be an URL)
serves        [set] of [dataset] Datasets served via this service

Field `url` SHOULD be used to provide a landing page or information about the service.

The first element of array field `type`, if given, MUST be the [item type] URI <http://www.w3.org/ns/dcat#DataService>.

::: {.callout-note}
Services roughly cover the [class DataService](https://www.w3.org/TR/vocab-dcat-3/#Class:Data_Service) from [Data Catalog Vocabulary](https://www.w3.org/TR/vocab-dcat-3/). A [dataset] being made available via a service can be linked:

a) via dataset field `services`
b) via dataset field `distributions` and [distribution] field `services`
c) via service field `serves`
:::

Field `serves` is not the direct inverse of [dataset] field `services`, but applications SHOULD imply the following: if `dataset:A` includes `service:B` in the set of its field `services`

~~~json
{
  "uri": "dataset:A",
  "services": [ { "uri": "service:B" } ]
}
~~~

than the `serves` set of `service:B` either contains `dataset:A`

~~~json
{
  "uri": "service:B",
  "serves": [ { "uri": "service:A" } ]
}
~~~

or it contains another dataset being a version of `dataset:A`:

~~~json
{
  "uri": "service:B",
  "serves": [ { "versionOf": [ { "uri": "service:A" } ] } ]
}
~~~

```{.json #lst-service lst-cap="A service to access a terminology via JSKOS API"}
{{< include examples/example.service.json >}}
```
...
A **concordance** is a [dataset] with the following fields (in addition to the
optional fields `@context`, `address`, `altLabel`, `annotations`, `changeNote`,
`contributor`, `created`, `creator`, `definition`, `depiction`, `distributions`,
`editorialNote`, `endDate`, `endPlace`, `example`, `guidelines`, `hiddenLabel`,
`historyNote`, `identifier`, `issue`, `issueTracker`,  `issued`, `location`,
`modified`, `notation`, `note`, `partOf`, `place`, `prefLabel`, `publisher`,
`rank`, `objectTypes`, `services`, `scopeNote`, `source`, `startDate`, `startPlace`, `subjectOf`,
`subject`, `tool`, `type`, `uri`, `url`, `version`, and `versionOf`). All
...
The first element of array field `type`, if given, MUST be the [item type] URI
<http://rdf-vocabulary.ddialliance.org/xkos#Correspondence>. Applications
SHOULD also support URI <http://rdfs.org/ns/void#Linkset> for backwards
compatability.

Field `objectTypes`, if given, SHOULD include the URI <http://rdf-vocabulary.ddialliance.org/xkos#Correspondence>.

::: {.callout-important title="Inference rules"}
For every [mapping] *M* of a [concordance] with `fromScheme` *S*, the set `fromScheme` of *M* also includes *S*.

For every [mapping] *M* of a [concordance] with `toScheme` *S*, the set `toScheme` of *M* also includes *S*.
:::

::: {.callout-note}
Concordances are used to collect multiple [mappings] that all share the same two [concept schemes]. For collections of mappings between arbitrary concept schemes use [registry] with field `objectTypes` including <http://rdf-vocabulary.ddialliance.org/xkos#Correspondence>.
:::
...
A (concept) mapping is a link between [concepts] of two [concept schemes]. It
consists of two [concept bundles] with additional metadata.

A **mapping** is an [item] with the following fields (in
addition to the optional fields `@context`, `address`, `altLabel`, `annotations`,
`changeNote`, `contributor`, `created`, `creator`, `definition`, `depiction`,
`editorialNote`, `endDate`, `endPlace`, `example`, `guidelines`, `hiddenLabel`,
`historyNote`, `identifier`, `issue`, `issueTracker`, `issued`, `location`,
...
`rank`, `scopeNote`, `source`, `startDate`, `startPlace`, `subjectOf`,
`subject`, `tool`, `type`, `uri`, `url`, `version`, and `versionOf`). All
fields except `from` and `to` are optional.
...
fromScheme       [concept scheme] source concept scheme
toScheme         [concept scheme] target concept scheme
mappingRelevance number           numerical confidence value between 0 and 1
justification    [uri]            action that resulted creation of the mapping
...
Values of field `justification` SHOULD be subclasses of
`https://w3id.org/semapv/vocab/Matching` from the Semantic Mapping Vocabulary.
...
::: {.callout-note}
Applications MAY use mappings to derive simple statements with [SKOS Mapping Properties](https://www.w3.org/TR/skos-reference/#mapping)
...
A dataset is an [item] being made available via [distributions] and/or [services].
A **dataset** is a JSON object with the following optional fields (in addition to the
optional fields `@context`, `address`, `altLabel`, `annotations`, `changeNote`,
`contributor`, `created`, `creator`, `definition`, `depiction`,
`editorialNote`, `endDate`, `endPlace`, `example`, `guidelines`, `hiddenLabel`,
`historyNote`, `identifier`, `issue`, `issueTracker`,  `issued`, `location`,
...
field         type                      definition
------------- ------------------------- ----------------------------------------------
distributions [set] of [distributions]  [distributions] to access the content of the dataset
services      [set] of [service]        Services to access the content of the dataset
extent        string                    Size of the concept scheme
license       [set]                     Licenses which the full scheme can be used under
objectTypes   [list] of [URI]           [object type URIs](#resource-and-property-types) of records in this dataset

Object subtypes of dataset include [concept scheme], [registry], and [concordance].

![dataset and connected object types](dataset.svg){#fig-dataset}

Field `objectTypes`, if given, MUST contain the following [object type URIs](#resource-and-property-types) for fields containing a non-empty [set]:

field        records            URI
------------ ------------------ -------
concepts     [concepts]         <http://www.w3.org/2004/02/skos/core#Concept>
schemes      [concept schemes]  <http://www.w3.org/2004/02/skos/core#ConceptScheme>
mappings     [mappings]         <http://www.w3.org/2004/02/skos/core#mappingRelation>
concordances [concordances]     <http://rdf-vocabulary.ddialliance.org/xkos#Correspondence>
occurrences  [occurrences]      *not defined yet*
registries   [registries]       <http://www.w3.org/ns/dcat#Catalog>
types        [resource types]   <http://www.w3.org/2002/07/owl#Class>
properties   [property types]   <http://www.w3.org/2000/01/rdf-schema#Property>
annotations  [annotations]      <http://www.w3.org/ns/oa#Annotation>


## Concept Bundle

[concept bundles]: #concept-bundle
[concept bundle]: #concept-bundle

A concept bundle is a group of [concepts]. Concept bundles can be used for [mappings], [occurrences], and composed concepts.
...
Concept bundles are allowed to empty. Keys of a `memberRoles` object MUST be URIs and their values MUST be of type [set].

```{.json #lst-member-roles lst-cap="A composed concept"}
{{< include examples/memberRoles.concept.json >}}
```

::: {.callout-note}
Concept bundles can also be used to express [SKOS concept collections](https://www.w3.org/TR/skos-primer/#seccollections).
Use type URI <http://schema.vocnet.org/NonIndexingConcept> to differentiate collections (aka "node labels") and normal concepts.
Details how to process and display these special concepts depend on use cases and applications.
:::

```{.json #lst-member-roles lst-cap="A concept bundle encoding a SKOS concept collection"}
{{< include examples/collection.concept.json >}}
```
...
[same resources]: #resource-sameness
...
- [concepts], [concept schemes], [registries], [distributions], [concordances], and [mappings] are pairwise disjoint (SKOS integrity rule S9)
...
## Expansion of link templates

A [link template] can be expanded to an [URL] as following from the combination of

- a non-empty [set] of [concepts],
- an optional **separator**, and
- an optional [list] of wanted [language tags].

If no separator is given, applications MUST:

- either reject expansion,
- or reduce the set of concepts to its first member,
- or use a default value as separator.

The variables used for template expansion, as defined by [RFC 6570], are:

- Variable `uri` is set to the URIs (field `uri`) of each concept, concatenated with the separator
- Variable `notation` is set to the first notations (field `notation`) of each concept, concatenated with the separator
- Variable `prefLabel` is set to the labels of each concept, concatenated with the separator.
  A labels is derived from the [language map] in field `prefLabel` by getting the first matching language tag from the list of wanted language tags. 
  Applications MAY choose another label if there is no match.
- Variable `language` is set to the first of the list of language tags

Empty values MUST be ignored.

:::{#lst-ex-link-templates lst-cap="Link templates"}

Link template to search for a concept in Wikipedia:

    https://{language}.wikipedia.org/w/index.php?title=Special:Search&search={prefLabel}

Link into K10plus catalogue by a GND identifier

    https://opac.k10plus.de/DB=2.299/CLK?IKT=8549&TRM={notation}

Link into K10plus catalogue by one or more DDC notations (separator `"+and+"`)

    https://opac.k10plus.de/DB=2.299/CMD?ACT=SRCHA&IKT=3011&NOABS=Y&TRM={notation}
...
- J. Gregorio, R. Fielding, M. Hadley, M. Nottingham, D. Orchard: *URI Template*.
  RFC 6570, March 2012. <http://tools.ietf.org/html/rfc6570>
...
- N. Matentzoglu et. al: *Semantic Mapping Vocabulary (SEMAPV)*.
  January 2025. <https://mapping-commons.github.io/semantic-mapping-vocabulary/>
...
* F. Cotton: *XKOS: An SKOS extension for representing statistical classifications*.
  DDI Alliance, May 2019.
  <https://rdf-vocabulary.ddialliance.org/xkos.html>

* O. Harting, P. Champin, G. Kellogg, A. Seaborne: *RDF-star and SPARQL-star*.
  W3C Community Group Report, December 2012
...
The schemas allow arbitrary unknonw fields for upwards compatibility, so they won't catch typos in field names!
To forbid unknown fields, you can add `"unevaluatedProperties": false` to schemas of [object types] not having subtypes.

See NodeJS library [jskos-validate] for an implementation with some additional rules not covered by JSON Schema alone.
...
JSKOS has been designed for compatibility with RDF in general and with [SKOS]
in particular. Most basic JSKOS data should be convertible to SKOS RDF and vice
versa with a [JSON-LD context](#json-ld). Nevertheless there are cases and
features where either JSKOS or RDF is a simplification or where opinions differ
about the best way to model the same information in RDF. In particular:

- RDF Ontologies expressed with OWL and/or RDFS are expressed the same way like
  SKOS terminologies in JSKOS, except for an addition value `type` (e.g.
  <http://www.w3.org/2002/07/owl#Class>). This requires a distinction of cases
  when mapping JSKOS back to RDF.

- [JSKOS Mappings](#mapping) in RDF can be expressed as plain triples, as
  reified triples, as resources of their own right, or with [RDF-star]. The most
  suitable form depends on use case and application, so there is no "right" way.
  Recommended classes to express mappings as resources include 
  <http://rdf-vocabulary.ddialliance.org/xkos#ConceptAssociation> and
  <http://www.w3.org/2002/07/owl#Axiom>.

- Semantics of most JSKOS fields is independent from object type, so they directly
  map to an RDF property. Known examples include field `concepts` of [concept scheme]
  (`skos:inScheme` for SKOS, `rdfs:isDefinedBy` for ontologies), vs. field 
  `concepts` of [registry] (`dct:isPartOf`).

- The hierarchy of JSKOS [object types] (see @fig-types) does not imply a corresponding
  hierarchy of RDF classes.
...
[closed world statements] and without [language ranges](#language-range) to RDF.There is also a [list of namespace prefixes](prefixes.json).
...
* [occurrences], [registries], [concordances], [mappings] as first-class objects, and composed [concepts]
* [closed world statements]
...
[Object types] can be expressed with [concepts] from the following [concept scheme]
(available [from here](examples/object-types.scheme.json)):

~~~json
{{< include examples/object-types.scheme.json >}}
~~~
...
*See <https://github.com/gbv/jskos/issues> for open issues*

### 0.7.1 (2026-01-07) {-}

- Add [registry] field `annotations`
- Connect [distribution] and [service] via field `services`
- Make [service] field `endpoint` optional
- Add abstract object type [dataset]
- Add documentation of SKOS collections and non-normative appendix on [mapping to RDF]
- Improve JSON Schema files

### 0.7.0 (2025-11-18) {-}

- Change item type URI of registry from <http://purl.org/cld/cdtype/CatalogueOrIndex> to <http://www.w3.org/ns/dcat#Catalog>
- Change item type URI of concordance from <http://rdfs.org/ns/void#Linkset> for <http://rdf-vocabulary.ddialliance.org/xkos#Correspondence>
- Clarify semantics of metadata fields `created`, `issued`, `modified`, `creator`, `contributor` and `source` 
- Object types: align naming, add new type [service], make [annotation] a data type
- Add [link templates]
