...
Application SHOULD also support the following type URIs for additional subtypes of [concept]:

- <http://schema.vocnet.org/NonIndexingConcept> for SKOS collections
- <http://www.w3.org/2002/07/owl#Class> for RDF classes
- <http://www.w3.org/1999/02/22-rdf-syntax-ns#Property> for RDF properties
...
Field `objectTypes` SHOULD list [object type URIs](#resource-and-property-types) of entities included in the registry:

resource type URI                                           if it contains 
----------------------------------------------------------- ------------------
<http://www.w3.org/2004/02/skos/core#Concept>               [concepts] (`concepts`, `types`, `properties`)
<http://www.w3.org/2004/02/skos/core#ConceptScheme>         [concept schemes] (`schemes`)
<http://www.w3.org/2004/02/skos/core#mappingRelation>       [mappings] (`mappings`)
<http://rdf-vocabulary.ddialliance.org/xkos#Correspondence> [concordances] (`concordances`)
??? (not specified yet)                                     [occurrences] (`occurrences`)
<http://www.w3.org/ns/dcat#Catalog>                         [registries] (`registries`)
<http://www.w3.org/ns/oa#Annotation>                        [annotations] (`annotations`)

A registry limited to only one of the fields `concepts`, `schemes`, `mappings`,
`concordances`, and `occurrences`, `registries` is also called a **collection**
of the corresponding [item type]. Registry field `types` SHOULD include the
corresponding item type URI.
...
Conversion of JSKOS registries to RDF (see [appendix](#mapping-to-rdf))
requires a different value for field `concepts` to not infer the RDF property
`skos:inScheme` but `dct:isPartOf`.
:::
...
objectTypes   [list] of [URI]           URIs of types of records in this dataset
