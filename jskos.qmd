...
`subject`, `tool`, `type`, `uri`, `url`, `version`, `versionOf`, `memberSet`,
...
`publisher`, `source`, `type`, `uri`, `memberSet`, `memberList`,
...
`endPlace`, `example`, `guidelines`, `hiddenLabel`, `historyNote`,
`identifier`, `issue`, `issueTracker`,  `issued`, `location`, `modified`,
`notation`, `note`, `partOf`, `place`, `prefLabel`, `publisher`, `rank`,
`scopeNote`, `source`, `startDate`, `startPlace`, `subjectOf`, `subject`,
`tool`, `type`, `uri`, `url`, `version`, and `versionOf`):
...
field         type              definition
------------- ----------------- ------------------------------------------------------------------
api           [URI]             API type, for instance from BARTOC vocabulary API types vocabulary
endpoint      [URI]             API endpoint (SHOULD be an URL)
serves        [set] of [items]  Specific versions of items served via this API

Field `type` SHOULD be used to provide the technical type of the service.
...
`notation`, `note`, `partOf`, `place`, `prefLabel`, `publisher`, `rank`,
`scopeNote`, `source`, `startDate`, `startPlace`, `subjectOf`, `subject`,
`tool`, `type`, `uri`, `url`, `version`, and `versionOf`). All fields except
`fromScheme` and `toScheme` are optional.
