...
A **link template** is a Level 2 URI Template as defined in [RFC 6570]. This includes expressions such as `` {var} `` (simple string expansion), `` {+var} `` (reserved expansion), and `` {#var} `` (fragment expansion) to be replaced with the expanded value of a variable.
...
For instance the occurrence could give the
...
The value of field `url`, if missing, SHOULD be derived from field `template` and from concepts of the occurrence, if existing, by [RFC 6570] expansion with variables determines as following:

- Get a list of concepts from the [concept bundle] field `memberSet`, `memberList`, `memberChoice`, or `memberRoles`, respectively
- Variable `notation` is set to the first notation of each concept, combined with separator from occurrence field `separator`
- Variable `uri` is set to the URI of each concept, combined with separator from occurrence field `separator`
- Variable `prefLabel` is set to one of the labels in field `prefLabel` of each concept, combined with separator from occurrence field `separator`. Application SHOULD choose the label based on its language configuration, if applicable.
